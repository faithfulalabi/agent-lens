...
/**
 * Tally every drifting session, grouped by harness version, so a Claude Code
 * release that changes the format shows up as one number going 0 -> N.
 *
 ...
 *
 * Clean rows are skipped by the writer's own marker, the same test `has_drift`
 * uses. Per-session `counts` carry the raw object, extra keys included.
 */
