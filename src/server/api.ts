...
// The tally moved to `drift-report.ts` so `doctor` can reach it without loading
// `hono/streaming`. Re-exported unchanged: `aggregateDrift` and `DriftReport`
// are imported from this module by three test files and by the UI's types.
